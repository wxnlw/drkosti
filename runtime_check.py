...
css=(root/'styles.css').read_text().replace("@import url('https://fonts.googleapis.com/css2?family=Press+Start+2P&family=Inter:wght@400;500;700;800&display=swap');","")
...
html=html.replace('<script src="./app-config.js" defer></script>', f'<script>{config}</script>')
...
with sync_playwright() as pw:
    ...
    def swipes_board(n):
        b=page.locator('.board-item').bounding_box()
        for i in range(n):
            sx=b['x']+25; sy=b['y']+25+i*3; ex=sx+68; ey=sy+8
            page.mouse.move(sx,sy); page.mouse.down(); page.mouse.move(ex,ey,steps=6); page.mouse.up(); wait(130)
        wait(250)
    def taps_stove(n):
    ...
    open_chest(); labels=page.locator('.inventory-item__label').all_text_contents(); p('labels1', labels); assert any('Замоченный рис' in x for x in labels); page.locator('#inventoryClose').click(); wait(80)
    ...
    open_chest(); empty=page.locator('#inventoryGrid').text_content(); p('empty', empty); assert 'Сундук пуст' in empty; page.locator('#inventoryClose').click(); wait(80)
    tool('#lidTool'); hold_kazan(); taps_kazan(5); tool('#ladleTool'); page.locator('#plate').click(); wait(520)
    st=state(); p('final state', st)
    assert st.get('completed') is True
    plate=page.locator('#plate').bounding_box(); food=page.locator('#plateFood').bounding_box();
    dx=abs((plate['x']+plate['width']/2)-(food['x']+food['width']/2)); dy=abs((plate['y']+plate['height']/2)-(food['y']+food['height']/2));
    p('center delta', dx, dy)
    assert dx <= 8 and dy <= 12
    p('ok')
    os._exit(0)

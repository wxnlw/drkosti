...
with sync_playwright() as pw:
    ...
    print('after soak', state(), flush=True)
    pick('Морковь')
    print('after carrot select', state(), flush=True)
    print('board count', page.locator('.board-item').count(), flush=True)
    print('inventory hidden', page.locator('#inventoryModal').evaluate("el=>el.classList.contains('is-hidden')"), flush=True)
    print('board html', page.locator('#boardSurface').inner_html(), flush=True)
    os._exit(0)
